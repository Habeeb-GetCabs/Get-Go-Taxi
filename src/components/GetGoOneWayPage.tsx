        ...
        <div className="lg:col-span-5 bg-white p-6 rounded-2xl border border-slate-200 shadow-md space-y-4">
          ...
          <form onSubmit={handleOneWaySubmit} className="space-y-4">
            ...
            <div>
              ...
              >
                <option value="sedan">AC Dzire / Etios (4 Seater) — Instant Online Quote</option>
                ...
                <option value="tempo">Tempo Traveller (12 Seater) — Call/WhatsApp for Rates</option>
              </select>
            </div>
            <div>
            ...
            <div className="space-y-3 pt-2">
              <div className="p-3 bg-red-50 border border-red-200 rounded-xl flex items-center justify-between">
                <div>
                  <span className="text-3xs font-black uppercase text-slate-500 block">Estimated Final Fare ({vehicle.toUpperCase()})</span>
                ...
                <div className="text-right space-y-1">
                  ...
                  <div>
                    <span className="text-3xs bg-amber-100 border border-amber-300 px-2 py-0.5 rounded font-bold text-amber-900 max-w-[180px] text-right inline-block">
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {POINT_TO_POINT_ROUTES.slice(0, 6).map((route) => {
              const routeSedan = route.sedanFareEst;
              const routeInnova = Math.round((route.distanceKm * 20) + 600);
              return (
                <div key={route.id} className="p-4 rounded-2xl bg-white border border-slate-200 shadow-xs hover:border-[#C62139] transition space-y-2">
                  ...
                  <div className="flex items-center justify-between border-t border-slate-100 pt-2 text-xs">
                    ...
                    <span className="font-extrabold text-[#C62139]">₹{routeSedan.toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-500 font-medium">Innova / SUV:</span>
                    <span className="font-extrabold text-slate-900">₹{routeInnova.toLocaleString()}</span>
                  </div>
                  <button
                    onClick={() => {
